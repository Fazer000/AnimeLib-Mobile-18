...
public class FloatingBottomSheetUtils {
    ...
    /**
     * Настраивает BottomSheetDialog как парящую панель в стиле YouTube
     * (с равными отступами сверху и снизу в 10dp, закруглёнными углами 20dp).
     */
    public static void setupFloatingStyle(BottomSheetDialog dialog) {
        setupFloatingStyle(dialog, false, false, R.drawable.bs_background);
    }

    public static void setupFloatingStyle(BottomSheetDialog dialog, boolean fullWidthWithMargins, int backgroundResId) {
        setupFloatingStyle(dialog, fullWidthWithMargins, false, backgroundResId);
    }
    ...
    public static void setupFloatingStyle(BottomSheetDialog dialog, boolean fullWidthWithMargins, boolean noSideMargins, int backgroundResId) {
        ...
        dialog.setOnShowListener(d -> applyFloatingToView(dialog, fullWidthWithMargins, noSideMargins, backgroundResId));
    }

    public static void applyFloatingToView(BottomSheetDialog dialog) {
        applyFloatingToView(dialog, false, false, R.drawable.bs_background);
    }
    ...
    public static void applyFloatingToView(BottomSheetDialog dialog, boolean fullWidthWithMargins, boolean noSideMargins, int backgroundResId) {
        ...
        if (bottomSheet != null) {
            ...
            int marginHorizontal = noSideMargins ? 0 : (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16, metrics);
            ...
            if (bottomSheet.getChildCount() > 0) {
                ...
                childView.setBackground(ContextCompat.getDrawable(context, bgDrawableRes));
                ...
                bottomSheet.setBackground(ContextCompat.getDrawable(context, bgDrawableRes));
            ...
            if (lp != null) {
                ...
                if (lp instanceof androidx.coordinatorlayout.widget.CoordinatorLayout.LayoutParams) {
                    ((androidx.coordinatorlayout.widget.CoordinatorLayout.LayoutParams) lp).gravity =
            ...
            // Блокируем передачу инсетов, чтобы клики и нажатия кнопок не вызывали смещение
